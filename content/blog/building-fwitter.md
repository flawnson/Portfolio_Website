...
So to fix all my problems, I built [Fwitter](/fwitter/): a personal microblog that lives on my domain, stores everything in my own database, and syndicates to social platforms automatically. The interesting part is the routing: instead of cross-posting the same thing everywhere, I use Gemini (because they have a generous free tier) to decide which single platform each fweet is best suited for, then route the content to that platform using its API.
...
The model gets the post's text and a short description of each platform's character, and it outputs exactly one word:

```text
You are a content routing agent for a personal microblog.
Given a post, choose the single best platform for it.

Platform guide:
- x: authority, startups, technical ideas, AI, systems thinking, founder energy.
  Sharp, high-signal, concise.
- bluesky: creativity, music, books, artistic observations, thoughtful reflection.
  Curious, nuanced, emotionally honest.
- threads: lifestyle, fitness, food, internet culture, casual daily life, warm humor.
  Casual, friendly, cozy.

Output EXACTLY ONE of: x, bluesky, threads
...
This section is a placeholder - I'll write it up properly once I've had more time with the frontend code.
